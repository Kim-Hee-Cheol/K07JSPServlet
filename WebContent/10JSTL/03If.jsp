...
<body>
	...
	if태그
		...
		-else문이 없기 땜누에 조건이 여려개일 때는 표현이 어렵다.
	...
	<!-- JSTL의 if문은 else가 없기 때문에 참 or 거짓의 형태로 만들 때는
		 아래와 같이 참인조건, 거짓인 조건으로 각각의 if문의 블럭을
		 만들어야한다.		
	-->
	<h3>문자열 비교</h3>
	<c:if test="${JAVA eq '자바' }">
		${strVar }는 자바다 <br/><!-- 조건이 false이므로 출력이 되지않음 -->
	</c:if>
	...
	-단, 일반값이더라도 TRUE(true)인 경우에는 true를 반환한다.
	 -->
	<h2>if문 사용시 주의사항</h2>
	...
	<!-- 
	아이디, 패스워드를 입력후 submit 한후 EL식을 통해 파라미터로 받은 후
